...
FLOPs up, bytes down. Local GB10 screening found a real memory saving and a clear
throughput regression ([D032](../docs/decisions.md#d032)); this measures both
numbers on another shape or stack. Arm order alternates between repetitions so
warmup or thermal drift cannot create a marginal result.
...
def main() -> None:
    ...
    parser.add_argument("--repetitions", type=int, default=2,
                        help="repeat variants and alternate order; must be positive")
    ...
    for name in args.sizes:
        ...
        variants = [("standard", base_settings)] + [
            (f"chunk {chunk}", replace(base_settings, chunked_cross_entropy=True,
                                        cross_entropy_chunk=chunk))
            for chunk in args.chunks]
        observations = {label: [] for label, _ in variants}
        for repetition in range(args.repetitions):
            ...
            for label, settings in order:
                observations[label].append(measure(
                    config, settings, steps=args.steps, warmup=args.warmup,
